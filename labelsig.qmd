...
Probably the simplest way to add stars to plots is by using the `ggsignif` package. This adds an extra layer to each `ggplot` chart containing the stars.
...
How to control number of decimal places? (probably just do annotation - see below)???
...
There are also several parameters within `geom_signif()` that can be tweaked to adjust the font type and position. This is using `textsize` to adjust the text size.
